 ...
 *
 * Run from the knote-app directory with: node upload_aconite.mjs
 */
...
async function uploadSymptom(sym, adminEmail) {
    ...
    console.log(`  ✅ Stored → ${imageUrl.slice(0, 90)}...`);
...
async function main() {
    ...
    if (!email || !password) {
        console.error('Usage: node upload_aconite.mjs <email> <password>');
        process.exit(1);
    }
    ...
    console.log('   Launch the game and go to Aconite nap → Easy to play!');
    process.exit(0);
}
